...
while true; do
    ...
    if [ "$OPTION" == "1" ]; then
        ...
        while true; do
            ...
            if [[ "$HOUR" =~ ^[0-9]{1,2}$ ]] && [ "$HOUR" -ge 0 ] && [ "$HOUR" -le 23 ]; then
                break
            else
        ...
        while true; do
            ...
            if [[ "$MINUTE" =~ ^[0-9]{1,2}$ ]] && [ "$MINUTE" -ge 0 ] && [ "$MINUTE" -le 59 ]; then
...
# -------------------------------------------
# بررسی وجود کرون جاب مشابه
# -------------------------------------------
CRON_JOB="bash $SCRIPT_PATH"
if crontab -l | grep -q "$CRON_JOB"; then
    echo "کرون جاب مشابهی پیدا شد. در حال حذف آن..."
    (crontab -l | grep -v "$CRON_JOB") | crontab -
fi

# تأیید نهایی از کاربر
read -p "آیا می‌خواهید کرون جاب را با این تنظیمات اضافه کنید؟ (y/n): " CONFIRM
if [[ "$CONFIRM" != "y" ]]; then
    echo "عملیات لغو شد."
    exit 1
fi
...
# اطلاع‌رسانی به کاربر در مورد مدیریت کرون جاب
echo "برای حذف یا ویرایش کرون‌جاب‌ها می‌توانید از دستور 'crontab -e' استفاده کنید."
